...
5. 点击“直接导入并预览”，确认材料、题目和参考答案候选后再保存套卷。

导入器会过滤非粉笔域名 Cookie，并且只有在同时解析到材料正文和题目时才允许导入。粉笔页面中的参考答案只作为未核验候选；AI 作答和批改仍应以原始材料、项目中的申论方法论和独立推导为准，不能把粉笔参考答案当作绝对正确答案。
