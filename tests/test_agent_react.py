...
from gongkao.agent_react import execute_tool, observation, tool_specs, validate_call, MAX_MODEL_CALLS, MAX_TOOL_CALLS
...
class ReactGraphTests(unittest.TestCase):
    def run_graph(self, responses, execute=None, **overrides):
        ...
        state = {"db_path": "unused", "run_id": 1, "task_type": "diagnosis", "user_goal": "分析近期训练", "subject_ids": []}
        state.update(overrides)
        with patch("gongkao.agent_graph._load_langgraph", return_value=(Mock(return_value=client), StateGraph, START, END)), \
             ...
             patch("gongkao.agent_graph.execute_tool", side_effect=execute or (lambda s, n, a: {"user_context": {"summary": {"attempt_count": 2}}})) as tool:
            graph = _graph_for({"model": "test", "api_base_url": "https://example.invalid/v1", "temperature": 0}, "unused")
    ...
    def test_parallel_calls_are_correlated_and_executed_in_order(self):
        _, client, tool, _ = self.run_graph([AIMessage(content="", tool_calls=[call(identifier="a"), call("retrieve_candidates", {"limit": 2}, "b")]), AIMessage(content="已整理建议。")])
    ...
    def test_repeated_call_uses_cache_but_still_counts_budget(self):
        result, _, tool, _ = self.run_graph([AIMessage(content="", tool_calls=[call(identifier="a")]), AIMessage(content="", tool_calls=[call(identifier="b")]), AIMessage(content="完成")])
        self.assertEqual(tool.call_count, 1)
        self.assertEqual(result["tool_calls_count"], 2)

    def test_invalid_arguments_return_observation_and_allow_recovery(self):
        result, client, _, _ = self.run_graph([AIMessage(content="", tool_calls=[call("unknown")]), AIMessage(content="请补充作答。")], execute=validate_call)
    ...
    def test_runaway_model_stops_at_budget_without_more_tools(self):
        result, client, tool, _ = self.run_graph([AIMessage(content="", tool_calls=[call(identifier=f"c{i}")]) for i in range(MAX_MODEL_CALLS)])
    ...
    def test_tool_budget_forces_final_round(self):
        result, client, _, _ = self.run_graph([AIMessage(content="", tool_calls=[call(identifier=str(i)) for i in range(MAX_TOOL_CALLS)]), AIMessage(content="根据已取得的资料整理。")])
    ...
    def test_invalid_tool_json_fails_without_execution(self):
        with self.assertRaises(AgentRunError):
            self.run_graph([AIMessage(content="", invalid_tool_calls=[{"name": "search_evidence", "args": "{", "id": "x", "error": "bad"}])])
...
class ToolContractTests(unittest.TestCase):
    ...
    def test_search_query_cannot_change_task_scope(self):
        state = {"db_path": "unused", "task_type": "review", "subject_ids": [7], "user_goal": "复盘这道题", "context_plan": {"rag_query_plan": {"scope": "current_attempt"}}}
        with patch("gongkao.agent_react.connect", return_value=nullcontext(Mock())), patch("gongkao.agent_react.build_rag_context", return_value={}) as build:
