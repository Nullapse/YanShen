...
批改原则：
1. 题干、作答要求和材料原文是最高事实来源。粉笔参考答案是主要参考和展示标杆，但不是脱离材料的绝对真理；其中无法由材料支持的发挥不得设为必得分点。
...
4. 申论采用踩点加分制：命中给分，未命中不给分，不倒扣。编造内容本身不额外倒扣，但占用篇幅并影响表达质量。
...
7. 粉笔遗漏但材料明确支持的内容，可以在诊断中标为“材料补充点”；不得据此另写一份标准答案。粉笔内容与材料冲突时，应标记争议并以材料为准。
...
## 采分点分析
使用表格列出：采分点、材料依据、粉笔参考、用户对应原句、命中/部分命中/未命中、得分理由。
...
## 参考答案可靠性说明
说明粉笔答案中经材料支持的要点、可能存在争议的内容，以及材料明确但粉笔遗漏的补充点。不得重复、改写或重新输出参考答案全文。
"""
...
def build_grading_package(
    ...
    parts.extend(
        [
            ...
            "- 【AI 仅评分诊断】：粉笔答案作为展示标杆；题干与材料为最高事实源。白鹭和小马哥只用于拆点、归并和同义识别，AI 禁止生成、改写或压缩完整答案。",
            "",
            "## 题目",
            question["prompt"],
            "",
            ...
            "## 本次批改参考答案",
            "",
            "参考答案使用规则：优先使用粉笔答案作为展示标杆；评分仍以题干、作答要求和材料原文为最高依据。AI 只做采分点判断和局部诊断，不得生成或改写完整答案。",
        ]
    )
