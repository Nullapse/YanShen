...
- 85%—100%用于任务完整、核心要点覆盖齐全、材料转化准确且结构清晰的优秀答案；不要求完美无缺，只要核心采分点覆盖充分、表达规范即可进入此档。
- 70%—84%用于主体任务完成较好、核心要点大部分覆盖、结构基本合理的中上答案；这是考场中大量合格考生的正常得分区间。
- 55%—69%用于主体任务基本成立但存在明显遗漏、结构松散或表达欠规范的中档答案。
- 40%—54%用于完成不充分、遗漏较多或结构表达明显影响阅卷识别的答案。
- 低于40%用于核心任务大面积缺失、严重偏题、文种/结构根本错误或表达大面积不可理解。
- 阅卷校准原则：当答案质量处于两档交界时，应归入较高档次。真实阅卷组在培训中反复强调宽容原则——只要考生表达了核心语义，即使措辞不够规范，也应认定为命中；不得因用语不如参考答案精练就降档。
- 不得因答案语句通顺就默认给结构、表达、格式高分；每个高于85%的维度都必须指出达到高分档的具体证据。
- 称谓、落款等仅在题干或相应文种确实要求时评分；题目不要求的要素缺失不得扣分。"""


ESSAY_SCORING_GUIDANCE = """综合写作必须先判断整篇文章所处的整体档位，再把该档位总分合理分配到各维度；不能先给每个维度“看起来不错”的高比例后相加。
- 80—100（一类卷）：立意准确深刻，紧扣主题和材料主旨，主要论证线均有充分展开和材料支撑，结构完整严密，表达规范流畅。允许存在个别细节瑕疵，不要求完美无缺。
- 70—79（二类卷）：立意正确，基本切合主题，结构较完整，主要论证较充分，材料运用基本准确。大多数考场合格答案应落入此区间。
- 60—69（三类卷）：立意基本正确但不够深刻，结构基本完整但层次感不强，论证有展开但部分段落偏薄或材料转化较简单。
- 50—59（四类卷）：基本切题但论证单薄、结构松散、材料运用不充分，或存在多处明显不足。
- 50以下：偏题、任务完成不充分，或论证结构存在严重缺陷。
阅卷校准原则：真实考场中大作文平均分通常在60-70分区间（按百分制），且阅卷组培训要求“就高不就低”。若立意正确、结构清楚且有材料支撑，即使论证不够精深，也不应低于60分。语言流畅和分段清楚虽不足以单独进入优秀档，但应在结构和表达维度中获得合理加分。"""
...
def build_grading_prompt(
    ...
    base_prompt = f"""你正在使用已校验的不等权评分基准执行申论综合批改。先分析全部采分点，再按评分基准中的固定维度综合评分。
...
"""
    if grading_prompt_template and grading_prompt_template.strip():
        return base_prompt + f"\n\n附加阅卷指令 (REPORT_INSTRUCTIONS)：\n{grading_prompt_template.strip()}\n"
    return base_prompt
